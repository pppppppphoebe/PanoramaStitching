...
def Spherical2Cartesian(rvh):
    r = rvh[0]
    v = math.radians(rvh[1])
    h = math.radians(rvh[2])
    
    return (r*math.sin(h)*math.cos(v), r*math.sin(h)*math.sin(v), r*math.cos(h))
...
class Camera:
    ...
    def update(self):
        ...
        gluLookAt(self.position[0], self.position[2], self.position[1],  self.center[0], self.center[1], self.center[2],  0.0, 1.0, 0.0)
